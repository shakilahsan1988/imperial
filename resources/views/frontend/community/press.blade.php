@extends('layouts.front')

@section('title', 'About Us - Imperial Health Bangladesh')

@section('content')

<!-- Press List Section -->
<section class="py-16 lg:py-24 bg-white">
    <div class="container mx-auto px-6">
        
        <!-- Header & Search Row -->
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-end mb-12">
            <!-- Title (Span 6) -->
            <div class="md:col-span-6">
                <h1 class="text-3xl md:text-4xl font-bold text-imperial-text font-sans">Press list</h1>
            </div>
            
            <!-- Search Box (Span 5, pushed to right) -->
            <div class="md:col-span-5 md:ml-auto">
                <div class="relative w-full">
                    <input type="text" placeholder="Search" class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-imperial-primary focus:border-imperial-primary transition shadow-sm text-gray-700">
                    <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                </div>
            </div>
        </div>

        <!-- Press Grid (2 Columns on Desktop) -->
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8">

            <!-- Item 1: Leadership Transition -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/NQFluTF0UZzV6qRqt7QKUwd6RB4=/538/fill-1280x748-c0/Mohammad_Abdul_Matin_Emon_2.jpg" alt="Mohammad Abdul Matin Emon" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">Mar 04, 2025</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">Praava Health Announces Leadership Transition</p>
                        </div>
                    </div>
                </a>
            </div>

            <!-- Item 2: Appoints Experts -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/jjNjsqLQUKYjitigXkcRlm_cpIU=/424/fill-1280x748-c0/Press_Praava_Health_Appoints.jpg" alt="Press Praava Health Appoints" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">Dec 13, 2022</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">Praava Health appoints 4 new industry experts to Global Advisory Council</p>
                        </div>
                    </div>
                </a>
            </div>

            <!-- Item 3: Bangladesh Finance MoU -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/idrLDKB0gWoFt-vTPRw8bxlMF0k=/405/fill-1280x748-c0/bfl_prava_photo.jpg" alt="bfl prava photo" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">Jun 09, 2023</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">Bangladesh Finance, Praava Health sign MoU on strategic partnership</p>
                        </div>
                    </div>
                </a>
            </div>

            <!-- Item 4: Welcomes COO -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/8yO1EgWffIACJgHRBwJr2-jZaLI=/494/fill-1280x748-c0/Website_Image_COO.png" alt="Website Image COO" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">Oct 29, 2024</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">Praava Health Welcomes Mohammad Abdul Matin Emon as Chief Operating Officer</p>
                        </div>
                    </div>
                </a>
            </div>

            <!-- Item 5: Partners with PHWC -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/NVhkBn59gKZu2-JukCk2M6gQt8s=/406/fill-1280x748-c0/praava-health---press-release-of-partnership-with-phwc.png" alt="Partnership with PHWC" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">May 09, 2023</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">Praava Health partners with Psychological Health and Wellness Clinic</p>
                        </div>
                    </div>
                </a>
            </div>

            <!-- Item 6: Ghore Lab -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/av0d6nOzdyKDVvrOv41sQi6vBac=/363/fill-1280x748-c0/ghorelab.jpg" alt="Ghore Lab" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">Feb 08, 2023</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">Praava Health launches home lab testing solution 'Ghore Lab'</p>
                        </div>
                    </div>
                </a>
            </div>

            <!-- Item 7: CME Molecular Cancer -->
            <div class="md:col-span-6">
                <a href="{{ route('press-details') }}" class="block group">
                    <div class="grid grid-cols-6 gap-4 h-full bg-white border border-gray-100 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div class="col-span-6 h-64 overflow-hidden">
                            <img src="https://www.praavahealth.com/media-images/bGvcjlDB8wRDDTK7tNr13_ooEb0=/425/fill-1280x748-c0/Press__CME_Molecular_Cancer.jpg" alt="CME Molecular Cancer" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-500">
                        </div>
                        <div class="col-span-6 p-4 flex flex-col justify-center">
                            <h3 class="text-sm font-bold text-gray-500 mb-1">Mar 11, 2017</h3>
                            <p class="text-lg font-bold text-imperial-text group-hover:text-imperial-primary transition leading-tight">CME: Molecular Cancer Diagnostics, cancer prevention, &amp; “Moonshot” program</p>
                        </div>
                    </div>
                </a>
            </div>

        </div>

        <!-- Optional: Load More Button -->
        <div class="flex justify-center mt-12">
            <button class="bg-white border border-gray-300 text-gray-700 font-semibold py-2 px-8 rounded hover:bg-gray-50 transition duration-300 focus:outline-none focus:ring-2 focus:ring-imperial-primary focus:ring-offset-2">
                Load More
            </button>
        </div>

    </div>
</section>
@endsection